...
function cambiarSong(direction) {
    audioIndex = (audioIndex + direction + list.size()) % list.size();
    cargarAudio(list.getElementAt(audioIndex).element);
    audioPlay(); 
}


function cargarAudio(song) {
    title.textContent = song.name;
    audio.src = song.audioSrc;
    cargarAlbumArt(song.image);
   
}

function cargarAlbumArt(image) {
